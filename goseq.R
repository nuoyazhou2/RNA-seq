...
for (file in list.files("../DESeq2/", "^DESeq2.*txt$")) {
  ...
  #   cat("--------------------------------------\n")
  # }
  genes2go=getgo(rownames(table[which(table$padj<cutoff),]),'mm9','geneSymbol')
  ...
  de_genes = select(org.Mm.eg.db, keys=as.vector(rownames(table[which(table$padj<=cutoff),])),
